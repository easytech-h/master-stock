...
const Sales: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl sm:text-3xl font-bold mb-8">Sales System</h1>
      {error && (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {selectedProducts.map((product) => (
                    <tr key={product.id}>
                      ...
                      <td className="px-4 py-2 whitespace-nowrap">${product.price.toFixed(2)}</td>
                      <td className="px-4 py-2 whitespace-nowrap">${(product.price * product.quantity).toFixed(2)}</td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <button
                      ...
                      {currentSale.items.map((item: any, index: number) => {
                        ...
                        return (
                          <tr key={index} className="border-b">
                            ...
                            <td className="text-right px-2 py-1">${item.price.toFixed(2)}</td>
                            <td className="text-right px-2 py-1">${(item.quantity * item.price).toFixed(2)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
